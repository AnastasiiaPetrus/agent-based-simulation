...
def build_llm_simulation_prompt(settings):
    ...
    return (
        ...
        "predictive justice. The goal is to illustrate trade-offs, not to optimize or justify policy.\n\n"
        "Non-negotiable framing:\n"
        "- This is not a real-world decision tool.\n"
        "- Do not claim to predict real people or real children.\n"
        "- Do not assign guilt, blame, dangerousness, deservingness, or moral status.\n"
        "- Do not recommend punishment, incapacitation, or real-world intervention.\n"
        "- Prediction is only an imperfect signal inside a synthetic scenario.\n"
        "- Districts A, B, and C are abstract labels only; they are not demographic groups, neighborhoods, "
        "protected classes, or proxies for real places.\n\n"
        "Policy definitions to follow exactly:\n"
        "- No action: baseline comparison only. No support, monitoring, coercion, intervention harm, "
        "cost, or crimes prevented.\n"
        "- Universal support: every synthetic child receives voluntary, non-punitive support. It may reduce "
        "modeled crime and creates support cost, but it should not create direct intervention harm.\n"
        "- Targeted support for high-risk children: only flagged synthetic children receive voluntary support. "
        "It may reduce modeled crime at lower cost than universal support, but it depends on an imperfect "
        "risk flag and false positives matter.\n"
        "- Surveillance of high-risk children: flagged synthetic children are monitored. It may modestly "
        "reduce modeled crime, but it creates privacy, stigma, and monitoring harm plus cost.\n"
        "- Coercive preventive intervention for high-risk children: flagged synthetic children face restrictive "
        "action before any act. It may reduce modeled crime most strongly, but must carry high harm and high "
        "ethical concern.\n"
        "- Rights-preserving targeted support: flagged synthetic children receive voluntary targeted support "
        "with protections against stigma and coercion. Any harm should be minimal and framed as residual "
        "stigma or administrative burden, not punishment.\n\n"
        "Fairness and bias handling:\n"
        "- bias_against_district_c is a sensitivity test, not a factual claim about a group.\n"
        "- If bias_against_district_c is 0, District A, B, and C should be broadly comparable except for "
        "small synthetic variation.\n"
        "- If bias_against_district_c is greater than 0, District C may show higher false positives and "
        "policy harm because the synthetic risk signal is biased against it.\n"
        "- Do not invent demographic explanations for District C. Explain uneven outcomes as model-bias "
        "sensitivity only.\n\n"
        "Metric meanings:\n"
        "- baseline_crimes: pre-policy modeled count.\n"
        "- crimes_after_policy: post-policy modeled count.\n"
        "- crimes_prevented: baseline_crimes minus crimes_after_policy.\n"
        "- false_positives: flagged synthetic children whose baseline outcome would not include the modeled offense.\n"
        "- false_negatives: unflagged synthetic children whose baseline outcome would include the modeled offense.\n"
        "- children_helped: synthetic children receiving voluntary support.\n"
        "- children_harmed: synthetic children receiving modeled intervention harm.\n"
        "- total_harm: aggregate harm created by the policy, not harm from baseline crime itself.\n"
        "- total_cost: aggregate resource cost created by the policy.\n\n"
        "Output requirements:\n"
        "- Return only valid JSON with exactly these keys: run_results, district_results, representative_agents, "
        "debrief_text.\n"
        ...
        "false_positives, harm, crimes.\n"
        "- representative_agents must contain only abstract synthetic children with no names, no protected "
        "attributes, no diagnoses, no family details, and no real-world identifiers.\n"
        ...
        f"- debrief_text must be no more than {DEFAULT_DEBRIEF_WORD_LIMIT} words and should explain the "
        "trade-off, uncertainty, false positives, false negatives, harm, cost, and District C bias if relevant. "
        "It must not recommend a policy as morally correct.\n\n"
